...
const quranSamples = [
  ...
  },
  {
    id: 7,
    label: 'Al-Fatihah 7',
    verse: 'صِرَ[n[َٰ]طَ [h:8[ٱ]لَّذِينَ أَنْعَمْتَ عَلَيْهِمْ غَيْرِ [h:9[ٱ]لْمَغْضُوبِ عَلَيْهِمْ وَلَا [h:10[ٱ][l[ل]ضَّ[m[َا]ٓلّ[p[ِي]نَ',
  },
  {
    id: 8,
    label: 'Al-Baqarah 1',
  ...
  },
  {
    id: 9,
    label: 'Al-Baqarah 2',
    verse: 'ذَ[n[َٰ]لِكَ [h:11[ٱ]لْكِتَٰبُ لَا رَيْبَۛ فِيهِۛ هُ[u:12[دًى ل]ِّ لْمُتَّقِ[p[ِي]نَ',
  },
];
...
export default function TajweedVerseScreen() {
  ...
  const [selectedFont, setSelectedFont] = useState<'system' | 'amiri' | 'amiri-quran' | 'noto' | 'scheherazade' | 'mirza' | 'harmattan' | 'katibeh' | 'handjet' | 'reem-fun' | 'reem-ink'>('amiri');
  const [selectedTheme, setSelectedTheme] = useState<'classic' | 'dark' | 'pastel' | 'accessible'>('classic');
  const [verseText, setVerseText] = useState<string>(quranSamples[0].verse);
  const [customInfoText, setCustomInfoText] = useState<string>('Tap a colored word to see the Tajweed rule explanation here!');
  const [customInfoTitle, setCustomInfoTitle] = useState<string>('Interactive Guide');
  ...
  const handleCustomRulePress = (ruleName: string, description: string, matchedText: string) => {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.subHeader}>Quranic text parsing, coloring, and interactions</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            ...
            <View style={[styles.row, { borderTopWidth: 1, borderTopColor: '#f1f5f9', paddingTop: 10, marginTop: 10 }]}>
              <Text style={styles.rowLabel}>Interactive Guides (interactive)</Text>
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sampleSelector}>
            {([
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.description}>Select an Ayah containing real database tags (e.g. [h:1[ٱ])</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sampleSelector}>
            {quranSamples.map((sample) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setCustomInfoText('Tap a colored word to see the Tajweed rule explanation here!');
                }}
              >
                <Text style={[
                  styles.sampleButtonText,
        ...
        <View style={styles.section}>
          ...
          <View style={styles.themeSelector}>
            {(['classic', 'dark', 'pastel', 'accessible'] as const).map((theme) => (
              <TouchableOpacity
                key={theme}
                style={[
                  styles.themeButton,
        ...
        {/* --- Live Preview Screen --- */}
        <View style={[styles.section, selectedTheme === 'dark' && styles.sectionDarkBg, { borderRadius: 24, padding: 15 }]}>
          <Text style={[styles.sectionTitle, selectedTheme === 'dark' && { color: '#e2e8f0' }]}>Live Preview</Text>
          
          <View style={[styles.previewCard, selectedTheme === 'dark' && { backgroundColor: '#1e293b', borderColor: '#334155' }]}>
            <TajweedVerse
              verse={verseText}
              colored={isColored}
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.description}>Demonstrates custom rules (e.g. bold underline styling for specific words)</Text>
          ...
          <View style={styles.previewCard}>
            <TajweedVerse
              ...
              customRules={[
                {
                  pattern: /\[custom\[([^\]]+)\]/,
                  style: { color: '#6366f1', fontWeight: 'bold', textDecorationLine: 'underline' },
                  renderText: (text: string) => text.replace(/\[custom\[/, '').replace(/\]/, ''),
                  onPress: (text: string) => Alert.alert('Custom Highlight Rule', `Tapped custom matched text: ${text}`)
                }
              ]}
            />
          </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            <Text style={styles.perfText}>
              The TajweedVerse component is memoized using React.memo. When other parts of the parent component update (like the counter below), the verse is NOT re-parsed.
            </Text>
            <Text style={styles.counterText}>Parent Render Check: {renderCount}</Text>
            <TouchableOpacity 
