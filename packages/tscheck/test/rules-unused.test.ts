...
describe("rules/unused", () => {
  it(
    "detects unused variables, parameters, and unused imports",
    () => {
      const code = `
  ...
  it(
    "handles suppressed unused diagnostics",
    () => {
      const code = `
      ...
      const { items, suppressedCount } = checkUnusedDiagnostics(
        program,
        sourceFile,
        "test-pkg",
        suppression
  ...
  it(
    "ignores variables and parameters prefixed with underscore",
    () => {
      const code = `
        export function example(_unusedParam: string, usedParam: number) {
          const _unusedVar = 100;
          return usedParam;
        }
      `;
