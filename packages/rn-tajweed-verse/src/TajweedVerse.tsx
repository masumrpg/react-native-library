...
export interface TajweedVerseProps {
   ...
   */
  onRulePress?: (ruleName: string, description: string, matchedText: string) => void;
  /**
...
export const TAJWEED_RULE_INFO: Record<keyof TajweedConfig, TajweedRuleInfo> = {
  ham_wasl: {
    name: 'Hamzatul Wasl',
    description: 'Connecting Hamzah. It is pronounced when beginning a sentence but dropped when connecting words.',
    pronunciation: 'Skip pronunciation when connecting (blend with the next letter).',
  },
  slnt: {
    name: 'Silent Letter',
    description: 'Letters written in the text but not pronounced during recitation.',
  ...
  madda_permissible: {
    ...
    description: 'Optional prolongation when a hamzah follows a madd letter in the next word.',
    pronunciation: 'Prolong the sound for 2, 4, or 5 beats.',
  },
  madda_necessary: {
    name: 'Necessary Madd (Madd Lazim)',
    description: 'Compulsory prolongation due to a sukoon or shaddah following a madd letter in the same word.',
    pronunciation: 'Prolong the sound for exactly 6 beats.',
  },
  qlq: {
    name: 'Qalqalah',
    description: 'Echoing or bouncing sound produced when reciting specific letters (ق, ط, ب, ج, د) with sukoon.',
    pronunciation: 'Make a quick echoing or rebounding sound at the end of the letter.',
  },
  madda_obligatory: {
    name: 'Obligatory Madd (Madd Wajib)',
    description: 'Compulsory prolongation when a hamzah follows a madd letter within the same word.',
    pronunciation: 'Prolong the sound for 4 or 5 beats.',
  },
  ikhf_shfw: {
    name: 'Ikhfa Syafawi',
    description: 'Hiding the sound of Meem Sakinah followed by Ba by pronouncing it with nasalization (ghunnah).',
    pronunciation: 'Pronounce Meem softly with a 2-beat nasal tone while holding the lips closed.',
  },
  ikhf: {
    name: 'Ikhfa',
    description: 'Concealing the sound of Noon Sakinah or Tanween when followed by one of the 15 Ikhfa letters.',
    pronunciation: 'Conceal Noon Sakinah in the nasal cavity for 2 beats before transitioning to the next letter.',
  },
  idghm_shfw: {
    name: 'Idgham Syafawi (Idgham Mithlayn)',
    description: 'Merging Meem Sakinah into a following Meem with nasalization.',
    pronunciation: 'Merge the Meem sounds cleanly and hold with a nasal sound for 2 beats.',
  },
  iqlb: {
    name: 'Iqlab',
    description: 'Converting Noon Sakinah or Tanween into a Meem sound when followed by the letter Ba.',
    pronunciation: 'Change the Noon sound to a soft Meem sound and hold for 2 beats with nasalization.',
  },
  idgh_ghn: {
    name: 'Idgham Ma\'al Ghunnah',
    description: 'Merging Noon Sakinah or Tanween into one of the letters (ي, ن, م, و) with nasalization.',
    pronunciation: 'Merge the Noon completely and prolong the nasal sound for 2 beats.',
  },
  idgh_w_ghn: {
    name: 'Idgham Bighairil Ghunnah',
    description: 'Merging Noon Sakinah or Tanween into the letters (ل, ر) without nasalization.',
    pronunciation: 'Merge the Noon completely into the next letter with no nasal hold.',
  },
  idgh_mus: {
    name: 'Idgham Mutamathilayn/Mutajanisayn',
    description: 'Merging of two identical or closely related consonants meeting each other.',
    pronunciation: 'Merge the first letter into the second and pronounce it with a shaddah.',
  },
  ghn: {
    name: 'Ghunnah',
    description: 'Nasalization sound emitted through the nose, specifically on a Noon or Meem with a shaddah.',
...
export const TajweedThemes = {
  ...
  accessible: {
    ...
    madda_normal: { style: { color: '#0055D4', fontWeight: 'bold' }, onPress: undefined },
    madda_permissible: { style: { color: '#0043A6', fontWeight: 'bold' }, onPress: undefined },
    madda_necessary: { style: { color: '#002C6C', fontWeight: 'bold' }, onPress: undefined },
    qlq: { style: { color: '#C60000', fontWeight: 'bold' }, onPress: undefined },
    madda_obligatory: { style: { color: '#0074E4', fontWeight: 'bold' }, onPress: undefined },
    ikhf_shfw: { style: { color: '#9B007F', fontWeight: 'bold' }, onPress: undefined },
    ikhf: { style: { color: '#73005F', fontWeight: 'bold' }, onPress: undefined },
    idghm_shfw: { style: { color: '#2F6C00', fontWeight: 'bold' }, onPress: undefined },
    iqlb: { style: { color: '#0097D4', fontWeight: 'bold' }, onPress: undefined },
    idgh_ghn: { style: { color: '#00835D', fontWeight: 'bold' }, onPress: undefined },
    idgh_w_ghn: { style: { color: '#217300', fontWeight: 'bold' }, onPress: undefined },
    idgh_mus: { style: { color: '#6A6A6A', fontWeight: 'bold' }, onPress: undefined },
    ghn: { style: { color: '#B54900', fontWeight: 'bold' }, onPress: undefined },
  } as Required<TajweedConfig>,
};
...
function TajweedVerse({
   ...
   */
  const handleRulePress = (ruleKey: keyof TajweedConfig, matchedText: string) => {
  ...
  const parser: ParseShape[] = [
    ...(customRules || []),
    {
      ...
      renderText,
      onPress: getOnPressHandler('ham_wasl', mergedConfig.tajweed.ham_wasl?.onPress),
    },
    {
      pattern: /\[s\[([^\]]+)\]/,
    ...
    },
    {
      ...
      renderText,
      onPress: getOnPressHandler('madda_normal', mergedConfig.tajweed.madda_normal?.onPress),
    },
    {
      pattern: /\[p\[([^\]]+)\]/,
      style: getRuleStyle(mergedConfig.tajweed.madda_permissible?.style),
      renderText,
      onPress: getOnPressHandler('madda_permissible', mergedConfig.tajweed.madda_permissible?.onPress),
    },
    {
      pattern: /\[m\[([^\]]+)\]/,
      style: getRuleStyle(mergedConfig.tajweed.madda_necessary?.style),
      renderText,
      onPress: getOnPressHandler('madda_necessary', mergedConfig.tajweed.madda_necessary?.onPress),
    },
    {
      pattern: /\[q\[([^\]]+)\]/,
    ...
    },
    {
      ...
      renderText,
      onPress: getOnPressHandler('madda_obligatory', mergedConfig.tajweed.madda_obligatory?.onPress),
    },
    {
      pattern: /\[c\[([^\]]+)\]/,
      style: getRuleStyle(mergedConfig.tajweed.ikhf_shfw?.style),
      renderText,
      onPress: getOnPressHandler('ikhf_shfw', mergedConfig.tajweed.ikhf_shfw?.onPress),
    },
    {
      pattern: /\[f\[([^\]]+)\]/,
    ...
    },
    {
      ...
      renderText,
      onPress: getOnPressHandler('idghm_shfw', mergedConfig.tajweed.idghm_shfw?.onPress),
    },
    {
      pattern: /\[i\[([^\]]+)\]/,
    ...
    },
    {
      ...
      renderText,
      onPress: getOnPressHandler('idgh_ghn', mergedConfig.tajweed.idgh_ghn?.onPress),
    },
    {
      pattern: /\[u\[([^\]]+)\]/,
      style: getRuleStyle(mergedConfig.tajweed.idgh_w_ghn?.style),
      renderText,
      onPress: getOnPressHandler('idgh_w_ghn', mergedConfig.tajweed.idgh_w_ghn?.onPress),
    },
    {
      pattern: /\[d\[([^\]]+)\]/,
      style: getRuleStyle(mergedConfig.tajweed.idgh_mus?.style),
      renderText,
      onPress: getOnPressHandler('idgh_mus', mergedConfig.tajweed.idgh_mus?.onPress),
    },
    {
      pattern: /\[b\[([^\]]+)\]/,
      style: getRuleStyle(mergedConfig.tajweed.idgh_mus?.style),
...
const MemoizedTajweedVerse = React.memo(TajweedVerse, (prevProps, nextProps) => {
  ...
  if (prevProps.customRules !== nextProps.customRules) {
    ...
    if (prevProps.customRules.length !== nextProps.customRules.length) return false;
  ...
  if (prevProps.config !== nextProps.config) {
    ...
    if (JSON.stringify(prevProps.config.style) !== JSON.stringify(nextProps.config.style)) return false;
    ...
    if (prevTajweed !== nextTajweed) {
      ...
      const keys = Object.keys(defaultConfig.tajweed || {}) as (keyof TajweedConfig)[];
      for (const key of keys) {
        if (JSON.stringify(prevTajweed[key]) !== JSON.stringify(nextTajweed[key])) {
          return false;
        }
      }
    }
