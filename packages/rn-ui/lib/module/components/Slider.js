...
export function Slider({
  ...
  const setInteractionActive = React.useCallback(active => {
    isInteractingRef.current = active;
  }, []);
  const commitValue = React.useCallback((nextValue, complete, start = false) => {
  ...
  const gesture = React.useMemo(() => Gesture.Pan().enabled(!disabled).minDistance(0).hitSlop(components.slider.hitSlop).onBegin(event => {
