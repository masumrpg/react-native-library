...
export function Breadcrumb({ style, ...props }: BreadcrumbProps) {
  ...
  return <View accessibilityRole="text" style={[{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: spacing.xs }, style]} {...props} />;
}
...
export function BreadcrumbLink({ children, disabled, ...props }: BreadcrumbLinkProps) {
  return (
    <Pressable disabled={disabled} {...props}>
      {typeof children === 'string' ? <Text variant="bodySmall" color={disabled ? 'text' : 'primary'}>{children}</Text> : children}
    </Pressable>
  );
}

export function BreadcrumbPage({ children }: { children?: React.ReactNode }) {
  return typeof children === 'string' ? <Text variant="bodySmall" color="text">{children}</Text> : <>{children}</>;
}

export function BreadcrumbSeparator({ children = '/' }: { children?: React.ReactNode }) {
  return typeof children === 'string' ? <Text variant="bodySmall" color="textSubtle">{children}</Text> : <>{children}</>;
}
