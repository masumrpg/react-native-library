        ...
        >
          ...
          <ScrollView keyboardShouldPersistTaps="handled">
            <View style={{ gap: spacing.sm }}>
              {filtered.length === 0 ? (
                <Text color="textMuted" align="center" style={{ padding: spacing.lg }}>{emptyText}</Text>
              ) : filtered.map((item) => (
                <Pressable
                  key={item.value}
                ...
                >
                  ...
                  <View style={{ flex: 1, gap: spacing.xs }}>
                    ...
                    {item.description ? <Text variant="bodySmall" color="textMuted">{item.description}</Text> : null}
                  </View>
                </Pressable>
              ))}
            </View>
